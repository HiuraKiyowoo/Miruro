...
// Decode HTML entities lalu strip semua tag HTML
function cleanDesc(str) {
  ...
  const decoded = str
    ...
    .replace(/&nbsp;/g, ' ')
    .replace(/&#\d+;/g, '');
  // Strip semua HTML tag termasuk atribut seperti bis_skin_checked
...
const MangaDetail = () => {
  ...
  useEffect(() => {
    ...
    apiFetch(`/manga/${slug}`)
      .then(res => { setManga(res.data); document.title = `${res.data.title} - Miruro`; })
      .catch(() => {})
      ...
      <div className="relative z-10 pt-20 max-w-4xl mx-auto px-4 md:px-6">
        ...
        <div className="flex flex-col md:flex-row gap-6 mb-8">
          <div className="w-full md:w-48 shrink-0 flex justify-center md:justify-start">
            <img src={imgUrl(manga.cover_url)} alt={manga.title}
              className="w-44 aspect-[3/4] object-cover rounded-md shadow-[0_20px_60px_rgba(0,0,0,0.6)]" />
          </div>

          <div className="flex flex-col flex-1 text-center md:text-left">
            <h1 className="text-2xl md:text-3xl font-black text-white mb-1 leading-tight tracking-tighter">{manga.title}</h1>
            {manga.alt_titles && (
              <p className="text-white/30 text-[11px] mb-4 font-medium leading-snug">{manga.alt_titles}</p>
            )}

            {/* Badges */}
            <div className="flex flex-wrap items-center justify-center md:justify-start gap-2 mb-4">
              {manga.type && (
                <span className="bg-[#F6CF80] text-black text-[9px] px-2.5 py-1 rounded-sm uppercase font-black tracking-widest">
                  {manga.type}
                </span>
              )}
              {manga.status && (
                <span className="bg-white/10 text-white/80 text-[9px] px-2.5 py-1 rounded-sm uppercase font-bold border border-white/5">
                  {manga.status}
                </span>
              )}
              {manga.rating && (
                <span className="flex items-center gap-1 bg-[#fbbf24]/10 text-[#fbbf24] text-[9px] px-2.5 py-1 rounded-sm font-bold border border-[#fbbf24]/20">
                  <StarIcon className="w-2.5 h-2.5" />
                  {parseFloat(manga.rating).toFixed(2)}
                </span>
              )}
              {manga.views && (
                <span className="flex items-center gap-1 bg-white/5 text-white/60 text-[9px] px-2.5 py-1 rounded-sm font-bold border border-white/5">
              ...
              <div className="flex flex-wrap gap-1.5 justify-center md:justify-start mb-4">
                {genres.map(g => (
                  <span key={g.slug} onClick={() => navigate(`/explore?genre=${g.slug}`)}
                    className="bg-white/5 border border-white/10 px-3 py-1 rounded-full text-[10px] font-bold text-white/60 cursor-pointer hover:bg-[#F6CF80] hover:text-black hover:border-[#F6CF80] transition-all">
                    {g.name}
                  </span>
                ))}
              </div>
            ...
            {description && (
              <div className="mb-5 text-left">
                <p className={`text-white/55 text-xs leading-relaxed ${!showFullDesc ? 'line-clamp-4' : ''}`}>
                  {description}
                </p>
                {description.length > 200 && (
                  <button onClick={() => setShowFullDesc(p => !p)}
                    className="text-[#F6CF80] text-[10px] font-black mt-1 hover:underline">
        ...
        {/* Chapter List */}
        <div className="bg-[#16161a] rounded-md border border-white/5 p-4 md:p-6 shadow-xl mb-8">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-white font-black uppercase text-sm tracking-wider">
              Chapters ({chapters.length})
            </h3>
            <button onClick={() => setSortDesc(p => !p)}
              className="text-white/40 hover:text-[#F6CF80] text-[10px] font-black uppercase tracking-widest transition-colors flex items-center gap-1">
            ...
            {chapters.map(ch => {
              ...
              return (
                <div key={ch.id} onClick={() => handleRead(ch)}
                  className={`flex items-center justify-between px-4 py-3 rounded-sm cursor-pointer transition-all active:scale-[0.99] ${isLast
                    ? 'bg-[#F6CF80]/10 border border-[#F6CF80]/30'
                    : 'bg-[#0a0a0c] border border-white/5 hover:border-white/20 hover:bg-white/5'}`}>
                  <div className="flex items-center gap-3 min-w-0">
                    <span className={`font-black text-sm shrink-0 ${isLast ? 'text-[#F6CF80]' : 'text-white/80'}`}>
                      Ch. {ch.chapter_number}
                    </span>
                    {ch.title && (
                      <span className="text-white/40 text-xs font-medium truncate">{ch.title}</span>
                    )}
                    {isLast && (
                      <span className="text-[#F6CF80] text-[8px] font-black uppercase shrink-0 bg-[#F6CF80]/10 px-1.5 py-0.5 rounded-sm">
                        Terakhir
                      </span>
                    )}
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    {ch.views && (
                      <span className="text-white/25 text-[9px] font-bold">{fmtNum(ch.views)}</span>
                    )}
